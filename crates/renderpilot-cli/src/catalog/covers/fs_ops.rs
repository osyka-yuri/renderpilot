...
pub(crate) fn unlink_cover_file_best_effort(catalog_db_path: &Path, file_name: Option<String>) {
...
pub(crate) fn gc_orphan_cover_files(
    ...
    for entry in entries {
        let entry = entry.map_err(|error| {
            CliError::CoverIo(format!("could not read covers directory entry: {error}"))
        })?;
...
fn referenced_cover_file_names(storage: &SqliteStorage) -> Result<HashSet<String>, CliError> {
    Ok(storage
...
fn read_covers_dir(covers_dir: &Path) -> Result<fs::ReadDir, CliError> {
    fs::read_dir(covers_dir)
        .map_err(|error| CliError::CoverIo(format!("could not read covers directory: {error}")))
}
