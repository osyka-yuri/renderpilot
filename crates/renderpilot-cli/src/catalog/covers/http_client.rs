...
pub(crate) fn http_client() -> Result<Client, CliError> {
    http_client_with_config(HttpClientConfig::default())
}

pub(crate) fn http_client_with_config(config: HttpClientConfig) -> Result<Client, CliError> {
    Client::builder()
...
fn client_build_error(error: reqwest::Error) -> CliError {
    CliError::CoverDownloadFailed(format!("failed to build HTTP client: {error}"))
}
