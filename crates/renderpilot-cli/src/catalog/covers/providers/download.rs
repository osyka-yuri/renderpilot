...
/// pipeline (validation, install) keep the simpler `Result<_, CliError>`
/// contract.
enum AttemptError {
...
fn classify_status(status: StatusCode) -> Result<(), AttemptError> {
    ...
    if matches!(status, StatusCode::NOT_FOUND | StatusCode::GONE) {
        return Err(AttemptError::Permanent(CliError::CoverNotFound));
    }

    let cli_error =
        CliError::CoverDownloadFailed(format!("cover download failed with HTTP status {status}"));
...
mod tests {
    ...
    fn permanent_cover_not_found() -> AttemptError {
        AttemptError::Permanent(CliError::CoverNotFound)
    }

    fn transient_failure(message: &str) -> AttemptError {
        AttemptError::Transient(CliError::CoverDownloadFailed(message.into()))
    }
    ...
    fn retries_transient_failures_up_to_max_attempts() {
        ...
        assert!(matches!(error, CliError::CoverDownloadFailed(_)));
    ...
    fn does_not_retry_permanent_failures() {
        ...
        assert!(matches!(result, Err(CliError::CoverNotFound)));
