...
pub(crate) fn validate_cover_bytes(bytes: &[u8]) -> Result<CoverFormat, CliError> {
    validate_cover_size(bytes)?;

    if looks_like_markup(bytes) {
        return Err(CliError::UnsupportedCoverImageType);
    }

    detect_cover_format(bytes).ok_or(CliError::UnsupportedCoverImageType)
}
...
fn validate_cover_size(bytes: &[u8]) -> Result<(), CliError> {
    let len = bytes.len() as u64;

    if len > MAX_COVER_BYTES {
        return Err(CliError::CoverDownloadFailed(
