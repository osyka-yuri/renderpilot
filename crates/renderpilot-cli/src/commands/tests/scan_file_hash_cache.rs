...
fn first_scan_populates_sqlite_file_hash_cache() {
    ...
    scan_catalog_folder(folder.path(), "first scan should populate file_hash_cache");

    let storage = SqliteStorage::open(&db_path).expect("catalog sqlite should open");
...
fn rescan_unchanged_file_keeps_consistent_file_hash_cache_entry() {
    ...
    scan_catalog_folder(folder.path(), "first scan");
    let storage = SqliteStorage::open(&db_path).expect("catalog sqlite should open");
    let sha_once = cache_sha_for_dll(&storage, folder.path());

    scan_catalog_folder(folder.path(), "second scan");
    let storage = SqliteStorage::open(&db_path).expect("catalog sqlite should open");
...
fn scan_updates_sqlite_file_hash_cache_after_file_change() {
    ...
    scan_catalog_folder(folder.path(), "rescan after edit");
...
fn failed_scan_does_not_overwrite_existing_file_hash_cache_rows() {
    ...
    let error = catalog::scan_folder(folder.path().to_path_buf());
    assert!(
        error.is_err(),
...
fn cache_sha_for_dll(storage: &SqliteStorage, folder: &Path) -> renderpilot_domain::Sha256Hash {
