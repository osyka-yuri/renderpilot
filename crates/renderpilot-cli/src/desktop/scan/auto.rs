...
use crate::desktop::catalog::GameDetailsOutput;
use crate::desktop::utils::{normalized_path_string, to_json, JsonResult};
use crate::CliError;
...
pub fn scan_auto_libraries() -> JsonResult {
    ...
    if let Err(error) = crate::catalog::with_catalog_storage(|storage| {
        crate::catalog::prune_auto_scan_orphans(storage, &library_root_keys, &install_path_keys)
    }) {
...
fn assert_auto_scan_shared_state_send_sync() {
    ...
    assert_send_sync::<AutoScanBatch>();
}
...
impl AutoScanAccumulator {
    ...
    fn push_global_error(&mut self, label: GlobalErrorLabel, error: CliError) {
...
impl ScanErrorOutput {
    ...
    fn with_label(label: GlobalErrorLabel, error: CliError) -> Self {
        Self {
