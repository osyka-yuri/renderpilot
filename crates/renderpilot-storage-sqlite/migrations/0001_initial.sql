...
CREATE TABLE IF NOT EXISTS backups (
    ...
    CHECK (instr(original_path, '\') = 0),
    CHECK (instr(backup_path, char(0)) = 0),
    CHECK (instr(backup_path, '\') = 0),
    CHECK (sha256 IS NULL OR length(sha256) = 64),
    CHECK (sha256 IS NULL OR lower(sha256) = sha256),
...
CREATE INDEX IF NOT EXISTS idx_backups_game_created
    ON backups(game_id, created_at, id);
...
CREATE TRIGGER IF NOT EXISTS trg_backups_component_game_insert
BEFORE INSERT ON backups
FOR EACH ROW
WHEN NEW.component_id IS NOT NULL
BEGIN
    SELECT RAISE(ABORT, 'backups component game mismatch')
    WHERE NOT EXISTS (
        SELECT 1
...
CREATE TRIGGER IF NOT EXISTS trg_backups_component_game_update
BEFORE UPDATE OF game_id, component_id ON backups
FOR EACH ROW
WHEN NEW.component_id IS NOT NULL
BEGIN
    SELECT RAISE(ABORT, 'backups component game mismatch')
