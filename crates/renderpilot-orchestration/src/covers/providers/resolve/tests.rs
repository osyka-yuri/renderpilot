...
impl SourceOutcome {
    ...
    fn into_result(self) -> Result<Vec<u8>, ServiceError> {
        match self {
            Self::Bytes(bytes) => Ok(bytes.to_vec()),
            Self::Miss => Err(ServiceError::CoverNotFound),
        }
    }
}
...
impl MockCoverSourceBackend {
    ...
    fn next_result(queue: &RefCell<VecDeque<SourceOutcome>>) -> Result<Vec<u8>, ServiceError> {
        queue
            .borrow_mut()
            .pop_front()
            .unwrap_or(SourceOutcome::Miss)
...
fn assert_cover_not_found(result: &Result<Vec<u8>, ServiceError>) {
    assert!(
        matches!(result, Err(ServiceError::CoverNotFound)),
