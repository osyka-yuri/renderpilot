...
pub(super) fn try_gog_cdn(client: &Client, product_id: &str) -> Result<Vec<u8>, ServiceError> {
    let product_id = normalize_gog_product_id(product_id)?;
    let product = fetch_gog_product(client, product_id)?;

    let logo = product.logo().ok_or(ServiceError::CoverNotFound)?;
    let cover_url = gog_vertical_cover_url_from_logo(logo).ok_or(ServiceError::CoverNotFound)?;
...
fn normalize_gog_product_id(product_id: &str) -> Result<&str, ServiceError> {
...
fn fetch_gog_product(client: &Client, product_id: &str) -> Result<GogProduct, ServiceError> {
    let response = client
...
fn map_gog_product_status(status: StatusCode) -> ServiceError {
    match status {
        StatusCode::BAD_REQUEST | StatusCode::NOT_FOUND => ServiceError::CoverNotFound,
        _ => ServiceError::CoverDownloadFailed(format!("GOG product API returned HTTP {status}")),
    }
}
