...
/// Force-refreshes remote manifests (libraries, RenoDX, ReShade) subject to the
/// process-local cooldown gate. Always returns a report JSON (partial CDN
/// failures are encoded per-kind, not as a hard error).
