...
import { createGamesFilterPersistence } from '@features/games/games-filter-persistence';

vi.mock('@shared/api/desktop', () => ({
  setCatalogSetting: vi.fn(),
}));

const setCatalogSettingMock = vi.mocked(setCatalogSetting);

type PersistResult = Awaited<ReturnType<typeof setCatalogSetting>>;
...
describe('games-filter-persistence', () => {
  beforeEach(() => {
    vi.useRealTimers();
    setCatalogSettingMock.mockReset();
  });
  ...
  it('does not persist when state is not ready', async () => {
    const persistence = createGamesFilterPersistence();
  ...
  it('debounces search persistence and writes only latest state', async () => {
    ...
    const persistence = createGamesFilterPersistence({ debounceMs: 100 });
  ...
  it('flushes queued search persistence immediately', async () => {
    ...
    const persistence = createGamesFilterPersistence({ debounceMs: 1000 });
  ...
  it('does not persist queued search when disposed before timer fires', async () => {
    ...
    const persistence = createGamesFilterPersistence({ debounceMs: 200 });
  ...
  it('routes write errors to onPersistError callback', async () => {
    ...
    const persistence = createGamesFilterPersistence({ onPersistError });
